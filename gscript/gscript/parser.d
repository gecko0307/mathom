...
class Parser
{
   ...
   public:
    ...
    {
        writefln("%s(%s): %s", filename, lex.lastTokenLine, message);
    ...
    void parseModule()
    {
        ...
        while(token != "")
        {
            ...
            else if (token == "import")
            {
                ...
                {
                    string fname = moduleToPath(importName, srcExt);
    ...
    */
    ...
    {
        Expression result;
        if (token.isNumber || token.isString || token.isBoolean || token == "Null")
        {
            lex.readNext();
        ...
        else if (token == "func")
        {
            ...
            modul.functions[f.name] = f;
            uint address = f.id.to!uint;
            result = new FunctionRefExpression(address);
        }
        else if (token == "if")
        {
            lex.readNext(); //pop "if"
        ...
        else if (isIdentifier(token)) // variable
        {
            ...
            if (lex.current == "(")
            {
                lex.readNext(); // pop left parenthesis
                if (lex.current != ")")
                    result = parseFunctionCallExpression(context, varName, parseListExpression(context));
                else
                    result = parseFunctionCallExpression(context, varName, new ListExpression());
                lex.readNext(); // pop right parenthesis
            }
            else
            {
                ...
                Expression indexExpr;
                if (lex.current == "[")
                    indexExpr = parseIndexExpression(Context(context.block, varExpr));

                if (isAssignment(lex.current)) // Store variable
                {
                    result = new StoreExpression(varExpr, indexExpr);                   
                }
                else // Load variable
                {
                    result = new LoadExpression(varExpr, indexExpr); 
                }
            }
        }
        else
    ...
    FunctionCallExpression parseFunctionCallExpression(Context context, string name, ListExpression list)
    {
        if (name == "self")
        {
            uint argsNum = currentFunc.args.names.length;
        ...
        {
            uint argsNum = modul.functions[name].args.names.length;
        ...
        else if (modul.hasImportedFunction(name)) // imported function
        {
            ...
            if (funcs == 1)
            {
                ...
                uint argsNum = func.args.names.length;
                if (list.children.length != argsNum)
                    if (list.children.length == 0 || argsNum != 1)
...
bool isValidName(string s)
{
    ...
    switch (s)
    {
        case "and", "break", "continue", "do", "else", "for", "foreach", "func", "if", 
             ...
             "Float", "Array", "String", "Function", "Null",
             "true", "false":
            return false;
        default:
            if (s.find('.').empty)
