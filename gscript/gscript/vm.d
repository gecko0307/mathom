...
class VirtualMachine
{
    ...
    void traceback(string message)
    {
        ...
        while(basePointer != 0)
        {
            ...
            funcPos = cast(uint)stack.pop.asFloat;
            basePointer = cast(uint)stack.pop.asFloat;
            uint position = cast(uint)stack.pop.asFloat;
            uint numArgs = cast(uint)stack.pop.asFloat;
        }
    ...
    void runFunction(uint labelCode, Dynamic[] args = [])
    {
        basePointer = 0;
        funcPos = 0;
        ...
        for( ; position < code.length; position++)
        {
            ...
            switch(op)
            {
                ...
                case "is":
                    ...
                    {
                        uint typeId = cast(uint)y.asFloat;
                        if (typeId == 0) // float
                        {
                            if (x.type == Type.Reference)
                ...
                case "return":
                    ...
                    funcPos = cast(uint)stack.pop.asFloat;
                    basePointer = cast(uint)stack.pop.asFloat;
                    position = cast(uint)stack.pop.asFloat;
                    uint n = cast(uint)stack.pop.asFloat;
                    foreach(arg; 0..n)
                        stack.pop();
                    stack.push(retVal);
                    break;
                ...
                case "ref": // makes a reference from local frame address
                    int adr = cast(int)stack.pop.asFloat;
                ...
                case "index": // makes a reference from array element
                    int index = cast(int)stack.pop.asFloat;
                ...
                case "indIn": // pushes 1 if element by index is present, or 0 if not
                    ...
                    if (elem.type == Type.Float)
                        index = cast(uint)elem.asFloat;
                    else if (elem.type == Type.Reference)
                        index = cast(uint)elem.asRef.asFloat;
                    else
                        traceback("Wrong operand type for \"" ~ op ~ "\" operator");
                ...
                case "objcall": // pop object and call it as function
                    ...
                    {
                        uint labelCode = funcObj.asFunction.label;
                        assert (labelCode in label);
                        stack.push(Dynamic(position));
                        stack.push(Dynamic(basePointer));
                        stack.push(Dynamic(funcPos));
                ...
                default:
                    ...
                    {
                        string val;
                        formattedRead(op, "pushfunc %s", &val);
                        if (isNumber(val))
                        {
                            uint labelCode = val.to!uint;
                            stack.push(Dynamic(Function(labelCode, funcAddressMap[label[labelCode]])));
                        }
                        else
                    ...
                    else if (op in prog.hostFunctions) // call host function
                    {
                        ...
                        if (n > 0)
                        {
                            foreach(arg; 0..n)
                            {
                                ...
                                {
                                    int absAdr = cast(int)argVal.asFloat;
