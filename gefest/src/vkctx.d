...
class VulkanContext
{
    ...
    void initVulkan()
    {
        ...
        dvkInstance = new DvkInstance(name, 1, 1);
        
        version(Windows)
        {
            dvkSurface = new DvkWindowsSurface(dvkInstance, window);
        }
        version(linux)
        {
            // TODO
        ...
        VkCommandPoolCreateInfo cmd_pool_info =
        {
            ...
            pNext: null,
            flags: 0,
            queueFamilyIndex: dvkSurface.graphicsQueueNodeIndex,
        };
        auto res = vkCreateCommandPool(dvkInstance.device, &cmd_pool_info, null, &cmd_pool);
        ...
        {
            res = vkAllocateCommandBuffers(dvkInstance.device, &cmdInfo, &buffers[i].cmd);
            assert(res == VkResult.VK_SUCCESS);
        }
    ...
    {
        uint formatCount;
        auto res = vkGetPhysicalDeviceSurfaceFormatsKHR(dvkInstance.physicalDevice, dvkSurface.surface, &formatCount, null);
        assert(res == VkResult.VK_SUCCESS);
        debug writefln("formatCount: %s", formatCount);
        
        VkSurfaceFormatKHR[] surfFormats = new VkSurfaceFormatKHR[formatCount];
        res = vkGetPhysicalDeviceSurfaceFormatsKHR(dvkInstance.physicalDevice, dvkSurface.surface, &formatCount, surfFormats.ptr);
        assert(res == VkResult.VK_SUCCESS);
        ...
        debug writefln("color_space: %s", color_space);
        
        vkGetPhysicalDeviceMemoryProperties(dvkInstance.physicalDevice, &memory_properties);
    }
    ...
    void prepareBuffers()
    {
        ...
        auto res = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(dvkInstance.physicalDevice, dvkSurface.surface, &surfCapabilities);
        assert(res == VkResult.VK_SUCCESS);
        
        uint presentModeCount;
        res = vkGetPhysicalDeviceSurfacePresentModesKHR(dvkInstance.physicalDevice, dvkSurface.surface, &presentModeCount, null);
        assert(res == VkResult.VK_SUCCESS);
        
        VkPresentModeKHR[] presentModes = new VkPresentModeKHR[presentModeCount];
        res = vkGetPhysicalDeviceSurfacePresentModesKHR(dvkInstance.physicalDevice, dvkSurface.surface, &presentModeCount, presentModes.ptr);
        assert(res == VkResult.VK_SUCCESS);
        ...
        res = vkCreateSwapchainKHR(dvkInstance.device, &swapchainInfo, null, &swapchain);
        assert(res == VkResult.VK_SUCCESS);
        
        if (oldSwapchain != VK_NULL_HANDLE)
        {
            vkDestroySwapchainKHR(dvkInstance.device, oldSwapchain, null);
        }
        
        res = vkGetSwapchainImagesKHR(dvkInstance.device, swapchain, &swapchainImageCount, null);
        assert(res == VkResult.VK_SUCCESS);
        
        debug writefln("swapchainImageCount: %s", swapchainImageCount);
        
        VkImage[] swapchainImages = new VkImage[swapchainImageCount];
        res = vkGetSwapchainImagesKHR(dvkInstance.device, swapchain, &swapchainImageCount, swapchainImages.ptr);
        assert(res == VkResult.VK_SUCCESS);
        ...
        for (size_t i = 0; i < swapchainImageCount; i++)
        {
            ...
            res = vkCreateImageView(dvkInstance.device, &color_image_view, null, &buffers[i].view);
            assert(res == VkResult.VK_SUCCESS);
        }
    }
    ...
    void prepareDepth()
    {
        ...
        res = vkCreateImage(dvkInstance.device, &imageInfo, null, &depth.image);
        assert(res == VkResult.VK_SUCCESS);
        
        vkGetImageMemoryRequirements(dvkInstance.device, depth.image, &mem_reqs);        
        ...
        res = vkAllocateMemory(dvkInstance.device, &depth.mem_alloc, null, &depth.mem);
        assert(res == VkResult.VK_SUCCESS);
        
        res = vkBindImageMemory(dvkInstance.device, depth.image, depth.mem, 0);
        assert(res == VkResult.VK_SUCCESS);
        ...
        res = vkCreateImageView(dvkInstance.device, &viewInfo, null, &depth.view);
        assert(res == VkResult.VK_SUCCESS);
    }
    ...
    */
        ...
        auto res = vkCreateDescriptorSetLayout(dvkInstance.device, &descriptor_layout, null, &desc_layout);
        assert(res == VkResult.VK_SUCCESS);
        ...
        res = vkCreatePipelineLayout(dvkInstance.device, &pPipelineLayoutCreateInfo, null, &pipeline_layout);
        assert(res == VkResult.VK_SUCCESS);
    }
    ...
    void prepareRenderPass()
    {
        ...
        VkResult res = vkCreateRenderPass(dvkInstance.device, &rp_info, null, &render_pass);
        assert(res == VkResult.VK_SUCCESS);
    }
    ...
    void preparePipeline()
    {
        ...
        auto res = vkCreatePipelineCache(dvkInstance.device, &pipelineCacheInfo, null, &pipelineCache);
        assert(res == VkResult.VK_SUCCESS);
        ...
        res = vkCreateGraphicsPipelines(dvkInstance.device, pipelineCache, 1, &pipelineInfo, null, &pipeline);
        assert(res == VkResult.VK_SUCCESS);
    }
    ...
    void prepareDescriptorPool()
    {
        ...
        auto res = vkCreateDescriptorPool(dvkInstance.device, &descriptor_pool, null, &desc_pool);
        assert(res == VkResult.VK_SUCCESS);
    }
    ...
    void prepareDescriptorSet()
    {
        ...
        */
        ...
        auto res = vkAllocateDescriptorSets(dvkInstance.device, &alloc_info, &desc_set);
        assert(res == VkResult.VK_SUCCESS);
    ...
    void prepareFramebuffers()
    {
        ...
        for (uint i = 0; i < swapchainImageCount; i++)
        {
            ...
            auto res = vkCreateFramebuffer(dvkInstance.device, &fb_info, null, &framebuffers[i]);
            assert(res == VkResult.VK_SUCCESS);
        }
    }
    ...
    VkShaderModule prepareShaderModule(const void* code, size_t size)
    {
        ...
        auto res = vkCreateShaderModule(dvkInstance.device, &moduleCreateInfo, null, &_module);
        assert(res == VkResult.VK_SUCCESS);
                        ...
                        VkImageLayout new_image_layout)
    {
        ...
        if (cmd is null)
        {
            ...
            res = vkAllocateCommandBuffers(dvkInstance.device, &cmdInfo, &cmd);
            assert(res == VkResult.VK_SUCCESS);
    ...
    void flushInitCmd()
    {
        ...
        res = vkQueueSubmit(dvkInstance.deviceQueue, 1, &submit_info, nullFence);
        assert(res == VkResult.VK_SUCCESS); 

        res = vkQueueWaitIdle(dvkInstance.deviceQueue);
        assert(res == VkResult.VK_SUCCESS); 

        vkFreeCommandBuffers(dvkInstance.device, cmd_pool, 1, cmd_bufs.ptr);
        cmd = null;
    }
    ...
    {            
        ...
        vkDeviceWaitIdle(dvkInstance.device);
    ...
    void draw()
    {
        ...
        auto res = vkCreateSemaphore(dvkInstance.device, 
            &presentCompleteSemaphoreCreateInfo, null, &presentCompleteSemaphore);
        assert(res == VkResult.VK_SUCCESS); 
        
        res = vkAcquireNextImageKHR(dvkInstance.device, swapchain, ulong.max,
        ...
        res = vkQueuePresentKHR(dvkInstance.deviceQueue, &present);
        if (res == VkResult.VK_ERROR_OUT_OF_DATE_KHR)
        {
            // demo->swapchain is out of date (e.g. the window was resized) and
            // must be recreated:
        ...
        vkDestroySemaphore(dvkInstance.device, presentCompleteSemaphore, null);
    }
    ...
    void destroy()
    {
        ...
        for (size_t i = 0; i < swapchainImageCount; i++)
        {
            vkDestroyFramebuffer(dvkInstance.device, framebuffers[i], null);
        }
        
        vkDestroyDescriptorPool(dvkInstance.device, desc_pool, null);
        
        vkDestroyPipeline(dvkInstance.device, pipeline, null);
        vkDestroyPipelineCache(dvkInstance.device, pipelineCache, null);
        vkDestroyRenderPass(dvkInstance.device, render_pass, null);
        vkDestroyPipelineLayout(dvkInstance.device, pipeline_layout, null);
        vkDestroyDescriptorSetLayout(dvkInstance.device, desc_layout, null);
        
        //Delete textures here
        
        vkDestroySwapchainKHR(dvkInstance.device, swapchain, null);
        
        vkDestroyImageView(dvkInstance.device, depth.view, null);
        vkDestroyImage(dvkInstance.device, depth.image, null);
        vkFreeMemory(dvkInstance.device, depth.mem, null);
        ...
        for (size_t i = 0; i < swapchainImageCount; i++)
        {
            vkDestroyImageView(dvkInstance.device, buffers[i].view, null);
            vkFreeCommandBuffers(dvkInstance.device, cmd_pool, 1, &buffers[i].cmd);
        }
        
        vkDestroyCommandPool(dvkInstance.device, cmd_pool, null);
        vkDestroyDevice(dvkInstance.device, null);
